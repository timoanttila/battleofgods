<script lang="ts">
  ...
  $: if (state) {
    ...
    $hero = {
      ...
      description: '',
      title,
      pages: [{name: 'Profile', url: '/profile/login'}]
    }
  }
...
{#if message}
  ...
  <p><a class="bg-blue btn text-white" href="/" rel="home">Back to home</a></p>
{:else}
  <form on:submit|preventDefault class="grid">
    ...
    <div id="form-actions" class="flex justify-between">
      {#if state === 1}
        <button on:click={() => (state = 2)} class="bg-black">Register</button>
        <button on:click={() => login()} disabled={!Username || !Password || loading} class="bg-blue">Login</button>
      {:else if state === 2}
        <button on:click={() => (state = 1)} class="bg-black">Login</button>
        <button on:click={() => register()} disabled={!Username || !Nickname || !Password || loading} class="bg-blue">Register</button>
      {:else if state === 3}
        <button on:click={() => (state = 1)} class="bg-black">Login</button>
        <button on:click={() => resetPassword()} disabled={!Username || loading} class="bg-blue">Reset</button>
      {:else if state === 4}
        <button on:click={() => (state = 1)} class="bg-black">Login</button>
        <button on:click={() => newPassword()} disabled={!Username || !Password || !verificationCode || loading} class="bg-blue">Reset</button>
      {/if}
    </div>
